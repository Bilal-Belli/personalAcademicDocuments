CREATE DATABASE db_ligue_centrale;
USE db_ligue_centrale;
...
CREATE TABLE EquipeWilaya(
    idEquipe VARCHAR(100) NOT NULL,
    idWilaya INTEGER NOT NULL,
    FOREIGN KEY (idEquipe) REFERENCES Equipe(id),
    FOREIGN KEY (idWilaya) REFERENCES Wilaya(id)
);

CREATE TABLE MembreBureauLigue(
    id INTEGER,
    idWilaya INTEGER,
    nom VARCHAR(100) NOT NULL,
    prenom VARCHAR(100) NOT NULL,
